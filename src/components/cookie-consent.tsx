...
export function CookieConsent() {
  ...
  return <>
    ...
    {open ? <aside className="cookie-banner" aria-label={es ? "Preferencias de cookies" : "Cookie preferences"}><div><strong>{es ? "Opciones de privacidad" : "Privacy choices"}</strong><p>{es ? "Usamos almacenamiento esencial. La analítica opcional y las métricas de rendimiento solo se activan con tu permiso." : "We use essential storage. Optional analytics and performance metrics run only with your permission."}</p></div><div className="cookie-actions"><button onClick={() => choose("rejected")} className="button button-ghost">{es ? "Solo esenciales" : "Essential only"}</button><button onClick={() => choose("accepted")} className="button">{es ? "Permitir analítica" : "Allow analytics"}</button></div></aside> : null}
  </>;
}
