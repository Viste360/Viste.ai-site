...
export function Footer({ locale }: { locale: "en" | "es" }) {
  ...
  return <footer className="site-footer"><div className="shell footer-grid">
    ...
    <div><h2>{locale === "en" ? "Explore" : "Explorar"}</h2>{navigation[locale].slice(0,5).map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}<Link href={locale === "en" ? "/voice" : "/es/voz"}>VISTE Voice</Link><Link href={diagnostic}>{locale === "en" ? "Opportunity diagnostic" : "Diagnóstico de oportunidad"}</Link><Link href={questions}>{locale === "en" ? "Practical questions" : "Preguntas prácticas"}</Link></div>
