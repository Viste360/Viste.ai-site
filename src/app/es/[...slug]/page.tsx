...
export function generateStaticParams() { return [...allPages.filter((page) => page.locale === "es").map((page) => ({ slug: page.path.replace(/^\/es\//, "").split("/") })), ...legalPages.filter((page) => page.locale === "es").map((page) => ({ slug: page.path.replace(/^\/es\//, "").split("/") })), ...insights.map((insight) => ({ slug: insight.path.es.replace(/^\/es\//, "").split("/") })), { slug: ["recursos"] }, { slug: ["contacto"] }, { slug: demoPath.replace(/^\/es\//, "").split("/") }]; }
