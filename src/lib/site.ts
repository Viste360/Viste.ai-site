...
export function pageMetadata(input: { title: string; description: string; path: string; alternatePath: string; locale: Locale; noIndex?: boolean }): Metadata {
  ...
  return {
    ...
    openGraph: { title: input.title, description: input.description, url: input.path, type: "website", locale: input.locale === "en" ? "en_GB" : "es_ES", images: [{ url: "/og.png", width: 1200, height: 630 }] },
